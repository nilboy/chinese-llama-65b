...
def write_model(model_path, input_base_path, model_size, tokenizer, old_tokenizer):
    ...
    for tid in tqdm(range(len(old_tokenizer), len(tokenizer)), desc='init embedding...'):
        word = tokenizer.convert_ids_to_tokens(tid)
        input_embeddings = model.get_input_embeddings().weight.data
        new_word_embedding = get_new_embedding(word, old_tokenizer, input_embeddings)
        input_embeddings[tid] = new_word_embedding
        output_embeddings = model.get_output_embeddings().weight.data
        new_word_embedding = get_new_embedding(word, old_tokenizer, output_embeddings)
        output_embeddings[tid] = new_word_embedding
...
def merge_tokenizer(tokenizer_path):
    orig_m = sp_model.ModelProto()
    orig_model_path = os.path.join(tokenizer_path, 'tokenizer.model')
    orig_m.ParseFromString(open(orig_model_path, "rb").read())
    merge_pieces = []
    for piece in orig_m.pieces:
        merge_pieces.append(piece.piece)
    base_piece = copy.deepcopy(orig_m.pieces[1000])

    extend_tokens = []
    for codepoint in range(0x4E00, 0x9FFF + 1):
        char = chr(codepoint)
        extend_tokens.append(char)

    for token in extend_tokens:
        if token not in merge_pieces:
            merge_pieces.append(token)
            piece = copy.deepcopy(base_piece)
            piece.piece = token
            piece.score = 0.0
            orig_m.pieces.append(piece)
    print(len(orig_m.pieces))
    with open(os.path.join(tokenizer_path, 'tokenizer.model'), 'wb') as f:
